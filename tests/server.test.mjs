...
test('live server: bot counts, difficulty, joins, leaves, validation, modes',async()=>{
 ...
 const clients=[];
 function wait(c,predicate){const old=c.messages.find(predicate);if(old)return Promise.resolve(old);return new Promise((resolve,reject)=>{const timer=setTimeout(()=>{c.ws.off('message',on);reject(Error('Timed out waiting for server message'));},5000);function on(data){const m=JSON.parse(data);if(predicate(m)){clearTimeout(timer);c.ws.off('message',on);resolve(m)}}c.ws.on('message',on);});}
 ...
 try{
  ...
  const health=await fetch(`http://127.0.0.1:${port}/api/health`).then(r=>r.json());assert.equal(health.version,'1.6.1');
  ...
  a.messages=[];send(a,{type:'grenade'});send(a,{type:'grenade'});state=await wait(a,m=>m.type==='state'&&m.players.find(p=>p.id===bombJoined.id)?.grenades===0);assert.equal(state.grenades.length,1);await wait(a,m=>m.type==='explosion'&&m.kind==='grenade');
