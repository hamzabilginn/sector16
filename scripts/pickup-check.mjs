...
try{
 ...
 await page.locator('#nickname').fill('Pickup QA');await page.locator('#preferredTeam').selectOption('blue');await page.locator('#createButton').tap();await page.locator('[name=name]').fill('Supply QA');await page.locator('[name=mode]').selectOption('tdm');await page.locator('[name=map]').selectOption('docks');await page.locator('#botCount').fill('0');await page.locator('#createForm button[type=submit]').tap();await page.locator('#resume').tap();await page.locator('#touchControls').waitFor({state:'visible'});
