...
try{
 ...
 const pads=[[-20,'smg'],[-4,'m4'],[4,'awp'],[12,'shotgun'],[20,'deagle']].sort((a,b)=>Math.abs(initial.x-a[0])-Math.abs(initial.x-b[0])),target=pads[0];
 ...
 await page.waitForFunction(w=>pickupQA.body().weapon===w,target[1]);const equipped=await page.evaluate(()=>pickupQA.body());assert.equal(equipped.money,initial.money);assert.equal(equipped.ammo,target[1]==='awp'?10:target[1]==='shotgun'?8:target[1]==='deagle'?7:30);
 await page.waitForTimeout(500);assert.deepEqual(errors,[]);console.log('PASS: iPad touch pickup, actual server movement, weapon equipped, money preserved, animated supply scene without runtime errors');
