...
window.qa={audio:()=>{wakeAudio();return typeof audio?.createPanner==='function'},hurt:d=>combat.hurt(d),results:r=>combat.results(r)};`}));
 ...
 await page.locator('#practiceWeapon').selectOption('awp');await page.waitForFunction(()=>document.getElementById('weaponName').textContent==='AWP');await page.locator('#resume').tap();
 assert.equal(await page.evaluate(()=>qa.audio()),true);await page.evaluate(()=>qa.hurt({x:0,z:1}));await page.locator('#damageBearing').waitFor({state:'visible'});assert.equal(await page.locator('#markerControls').count(),0);
 ...
 assert.deepEqual(errors,[]);console.log('PASS: private training room, moving targets, weapon choice, removed marker controls, damage bearing and match statistics UI.');
