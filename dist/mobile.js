...
export function mobileControls({look,action,pause,score,shop}){
 ...
 return {state,reset,takeReload(){const value=state.reload;state.reload=false;return value;},setActive(value){value=!!value&&touchDevice;if(value!==enabled){reset();enabled=value;}root.hidden=!enabled;},context({bomb,dead,spectating=false,grenades}){for(const id of ['touchFire','touchAim','touchReload','touchWeapon'])document.getElementById(id).disabled=dead;document.getElementById('touchInteract').hidden=!bomb||dead;document.getElementById('touchGrenade').disabled=dead||!grenades;document.getElementById('touchJump').hidden=spectating;document.getElementById('touchCrouch').hidden=spectating;}};
}
