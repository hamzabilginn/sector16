...
export function buildArena(scene,id){const map=MAPS[id]||MAPS.docks;const BOXES=map.boxes;const root=new THREE.Group();scene.add(root);paintBases(root);scene.background=new THREE.Color(map.sky);scene.fog=new THREE.FogExp2(map.sky,.009);if(id!=='docks'){buildThemed(root,map);return root}
...
function paintBases(root){for(const [z,color]of [[28,0x60bbef],[-28,0xf39b63]]){const zone=new THREE.Mesh(new THREE.PlaneGeometry(55,8),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.13,depthWrite:false}));zone.rotation.x=-Math.PI/2;zone.position.set(0,.018,z);root.add(zone);const edge=new THREE.Mesh(new THREE.PlaneGeometry(55,.1),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.8,depthWrite:false}));edge.rotation.x=-Math.PI/2;edge.position.set(0,.021,z>0?24:-24);root.add(edge)}}
