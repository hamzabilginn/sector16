...
export function buildArena(scene,id){const map=MAPS[id]||MAPS.docks;const BOXES=map.boxes;const root=new THREE.Group();scene.add(root);paintBases(root,map);scene.background=new THREE.Color(map.sky);scene.fog=new THREE.FogExp2(map.sky,id==='city'?.003:.009);if(id==='city')return buildCity(root,map);if(id==='depot')return buildDepot(root,map);if(id!=='docks'){buildThemed(root,map);return root}
