...
export function tacticalVisuals(scene){
 ...
 function update(s){state=s;sites.visible=s?.mode==='bomb';bomb.visible=sites.visible&&['dropped','planted'].includes(s?.bomb?.state);if(bomb.visible)bomb.position.set(s.bomb.x,s.bomb.y,s.bomb.z);const ids=new Set();for(const g of s?.grenades||[]){ids.add(g.id);let mesh=grenades.get(g.id);if(!mesh){mesh=new THREE.Mesh(geo,mat);mesh.castShadow=true;root.add(mesh);grenades.set(g.id,mesh);}mesh.position.set(g.x,g.y,g.z);mesh.rotation.x+=.4;}for(const [id,g] of grenades)if(!ids.has(id)){root.remove(g);grenades.delete(id);}}
