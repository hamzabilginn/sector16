...
export function tacticalInput(p,r,time){
 ...
 const foes=[...r.players.values()].filter(e=>enemies(p,e,r.mode)&&e.hp>0);const targets=foes.map(e=>{const dx=e.x-p.x,dz=e.z-p.z,dist=Math.hypot(dx,dz),yaw=Math.atan2(-dx,-dz),pitch=Math.atan2(eye(e)-eye(p)-.2,dist);return {e,dist,yaw,pitch,visible:dist<level.range&&wallDistance({x:p.x,y:eye(p),z:p.z},direction(yaw,pitch),boxes)>dist-.3&&!smokeBlocks({x:p.x,y:eye(p),z:p.z},{x:e.x,y:eye(e),z:e.z},r.smokes,time)};}).sort((a,b)=>(a.visible?-100:0)+a.dist-((b.visible?-100:0)+b.dist));
 const target=targets[0];let yaw=p.yaw,pitch=0,fire=false,x=0,z=0,interact=false,sprint=false;
 let goal;
 if(r.mode==='escort'&&r.escort){const c=r.escort;goal=p.team==='orange'?{x:c.x+ai.lane*2.5,z:c.z-2}:{x:c.x+ai.lane*4,z:c.z+5};}
 else if(r.mode==='bomb'&&r.bomb){const b=r.bomb;if(b.state==='planted'){goal={x:b.x+(p.team==='orange'?ai.lane*6:0),z:b.z+(p.team==='orange'?-6:0)};if(p.team==='blue'&&Math.hypot(p.x-b.x,p.z-b.z)<1.6)interact=true;}
