...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/api/health')return json(res,200,{ok:true,game:'Sector 16',version:'2.1.0',rooms:rooms.size,players:[...peers].filter(p=>p.room).length,tickRate:60});
...
function tick(){clock+=1/60;for(const r of rooms.values()){
 ...
 if(!isRoundMode(r.mode)&&r.restart&&clock>=r.restart){r.matchId++;r.grenades=[];r.smokes=[];r.history=[];r.bomb=null;r.scores={blue:0,orange:0};r.flags=initialFlags(r.map);r.end=clock+r.minutes*60;r.restart=0;for(const p of r.players.values()){p.kills=0;p.deaths=0;p.captures=0;p.stats={shots:0,hits:0,headshots:0,damage:0};spawn(p,r)}broadcast(r,{type:'event',kind:'newmatch'})}
 if(!isRoundMode(r.mode)&&!r.restart&&(clock>=r.end||r.scores.blue>=modeLimit(r.mode)||r.scores.orange>=modeLimit(r.mode))){r.restart=clock+8;broadcast(r,{type:'end',winner:r.scores.blue===r.scores.orange?'draw':r.scores.blue>r.scores.orange?'blue':'orange'})}
