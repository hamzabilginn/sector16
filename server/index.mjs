import {takeAttachment,weaponStats} from '../shared/attachments.mjs';
...
import {freshEscort,stepEscort} from '../shared/escort.mjs';
...
import {LIVE_MODES,modeLoadout,modeLimit,enemies,smokeBlocks} from '../shared/live-expansion.mjs';
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/api/health')return json(res,200,{ok:true,game:'Sector 16',version:'2.2.0',rooms:rooms.size,players:[...peers].filter(p=>p.room).length,tickRate:60});
...
function spawn(p,r,preserve=false){
 ...
 const attachments=retained?p.attachments||{}:{};
 const inventory=Object.fromEntries(owned.map(id=>[id,{ammo:weaponStats(id,attachments).mag,reserve:WEAPONS[id].reserve}]));
 ...
 Object.assign(p,freshBody(pos[0]+(Math.random()-.5),pos[1],p.team==='blue'?0:Math.PI),{ai:p.bot?makeBrain(p.botIndex||0):undefined,medkits:1,grenades:3,utility:{he:1,flash:1,smoke:1},botTarget:null,botSeenAt:0,hp:100,armor:retained?p.armor:0,money:p.money??800,primary,secondary,weapon,inventory,attachments,attachmentAt:0,ammo:inventory[weapon].ammo,reserve:inventory[weapon].reserve,reloadAt:0,nextFire:clock+.5,invuln:clock+2,respawn:0,queue:[],input:{},generation:(p.generation||0)+1});
}
...
function beginRound(r,fresh=false){if(fresh){r.matchId++;r.smokes=[];r.round=0;r.scores={blue:0,orange:0};r.loss={blue:0,orange:0};for(const p of r.players.values()){p.money=800;p.hp=0;p.kills=0;p.deaths=0;p.stats={shots:0,hits:0,headshots:0,damage:0}}}
...
function melee(p,r){
 ...
 if(victim){p.stats.hits++;broadcast(r,{type:'impact',victimId:victim.id,armor:victim.armor>0,head:false,position:to,direction:d});dealDamage(p,victim,r,w.damage,false,p.weapon,d);}
}
function fire(p,r){const w=weaponStats(p.weapon,p.attachments);if(w.melee)return melee(p,r);p.stats??={shots:0,hits:0,headshots:0,damage:0};p.stats.shots++;let countedHit=false,countedHead=false;p.ammo--;p.inventory[p.weapon].ammo=p.ammo;p.nextFire=clock+w.interval;const spread=w.spread*(p.input?.aim?.65:1)+(Math.hypot(p.vx,p.vz)>1?.014:0)+(p.ground?0:.018);for(let pellet=0;pellet<(w.pellets||1);pellet++){const yaw=p.yaw+(Math.random()-.5)*spread,pitch=p.pitch+(Math.random()-.5)*spread;const o={x:p.x,y:eye(p),z:p.z},d=direction(yaw,pitch);let nearest=wallDistance(o,d,MAPS[r.map].boxes),victim=null,head=false;for(const v of r.players.values()){if(!enemies(p,v,r.mode)||v.hp<=0||v.invuln>clock)continue;const tested=rewoundTarget(p,r,v);if(smokeBlocks(o,{x:tested.x,y:eye(tested),z:tested.z},r.smokes,clock))continue;const hit=playerHit(o,d,tested,.12);if(hit.distance<nearest){nearest=hit.distance;victim=v;head=hit.head}}
 broadcast(r,{type:'shot',id:p.id,weapon:p.weapon,suppressed:w.suppressed,surface:victim?null:surfaceImpact(o,d,nearest,MAPS[r.map].boxes),from:o,to:{x:o.x+d.x*nearest,y:o.y+d.y*nearest,z:o.z+d.z*nearest}});
 if(victim){if(!countedHit){p.stats.hits++;countedHit=true}if(head&&!countedHead){p.stats.headshots++;countedHead=true}broadcast(r,{type:'impact',victimId:victim.id,armor:victim.armor>0,head,position:{x:o.x+d.x*nearest,y:o.y+d.y*nearest,z:o.z+d.z*nearest},direction:d});dealDamage(p,victim,r,head?w.head:w.damage,head,p.weapon,d);}
...
function tick(){clock+=1/60;for(const r of rooms.values()){
 ...
 if(!isRoundMode(r.mode)&&r.restart&&clock>=r.restart){r.matchId++;r.grenades=[];r.smokes=[];r.history=[];r.bomb=null;r.scores={blue:0,orange:0};r.flags=initialFlags(r.map);if(r.mode==='escort')r.escort=freshEscort(r.map);r.end=clock+r.minutes*60;r.restart=0;for(const p of r.players.values()){p.kills=0;p.deaths=0;p.captures=0;p.stats={shots:0,hits:0,headshots:0,damage:0};spawn(p,r)}broadcast(r,{type:'event',kind:'newmatch'})}
 if(!isRoundMode(r.mode)&&!r.restart&&(clock>=r.end||r.scores.blue>=modeLimit(r.mode)||r.scores.orange>=modeLimit(r.mode)||r.mode==='escort'&&r.escort?.completed)){r.restart=clock+8;broadcast(r,{type:'end',winner:r.mode==='escort'?(r.escort.completed?'orange':'blue'):r.scores.blue===r.scores.orange?'draw':r.scores.blue>r.scores.orange?'blue':'orange'})}
 ...
 const w=weaponStats(p.weapon,p.attachments);if(p.reloadAt&&clock>=p.reloadAt){const n=Math.min(w.mag-p.ammo,p.reserve);p.ammo+=n;p.reserve-=n;p.inventory[p.weapon]={ammo:p.ammo,reserve:p.reserve};p.reloadAt=0}
 ...
 if(r.mode==='escort'&&!r.restart){const event=stepEscort(r,1/60);if(event)broadcast(r,{type:'escort',kind:event});}
...
const snapshots=setInterval(()=>{for(const r of rooms.values()){if(!r.players.size)continue;const players=[...r.players.values()].map(p=>({id:p.id,name:p.name,team:p.team,bot:!!p.bot,role:p.bot?BOT_ROLES[(p.botIndex||0)%BOT_ROLES.length]:null,medkits:p.medkits,grenades:p.grenades,utility:p.utility,skin:p.profile?.skin||'standard',x:p.x,y:p.y,z:p.z,yaw:p.yaw,pitch:p.pitch,vx:p.vx,vy:p.vy,vz:p.vz,ground:p.ground,jumpHeld:p.jumpHeld,crouch:p.crouch,hp:p.hp,inBase:inOwnBase(p),canBuy:p.hp>0&&inOwnBase(p)&&!r.restart&&(!isRoundMode(r.mode)||r.phase!=='matchEnd'),armor:p.armor,money:p.money,captures:p.captures||0,primary:p.primary,secondary:p.secondary,weapon:p.weapon,attachments:p.attachments||{},ammo:p.ammo,reserve:p.reserve,reload:Math.max(0,p.reloadAt-clock),reloadEnd:p.reloadAt,kills:p.kills,deaths:p.deaths,ping:Math.round(p.ping),ack:p.ack,invuln:p.invuln>clock,respawn:Math.max(0,p.respawn-clock),generation:p.generation}));broadcast(r,{type:'state',matchId:r.matchId,time:clock,map:r.map,mode:r.mode,flags:r.mode==='ctf'?r.flags:undefined,escort:r.mode==='escort'?r.escort:undefined,bomb:r.mode==='bomb'?r.bomb:undefined,grenades:r.grenades.map(g=>({id:g.id,kind:g.kind,x:g.x,y:g.y,z:g.z})),smokes:r.smokes.filter(x=>x.until>clock),phase:r.phase,round:r.round,roundTarget:ROUND_TARGET,remaining:Math.max(0,(r.mode==='bomb'&&r.bomb?.state==='planted'?r.bomb.detonateAt:isRoundMode(r.mode)?r.phaseEnd:r.end)-clock),restart:Math.max(0,r.restart-clock),scores:r.scores,players})}},1000/20);
...
makeRoom({name:'Ray Hattı / Araç Eskortu',permanent:true,mode:'escort',map:'depot',maxPlayers:12,botCount:7});
...
makeRoom({name:'Karmaşa Şehri / Takım Savaşı',permanent:true,mode:'tdm',map:'city',maxPlayers:16,botCount:11});
