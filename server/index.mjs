...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/api/health')return json(res,200,{ok:true,game:'Sector 16',version:'1.6.0',rooms:rooms.size,players:[...peers].filter(p=>p.room).length,tickRate:60});
...
wss.on('connection',(ws,req)=>{const p={id:crypto.randomBytes(8).toString('hex'),ws,ip:clientIp(req),room:null,alive:true,ping:0,queue:[],input:{},ack:0,lastSeq:0,busy:false};peers.add(p);ws.on('pong',()=>p.alive=true);send(p,{type:'hello',id:p.id,rooms:roomList()});ws.on('message',async data=>{let m;try{m=JSON.parse(data);if(!m||typeof m.type!=='string')return;
 ...
 consume(p,'messages',60,10);if(m.type==='trainingweapon'){if(p.room?.mode!=='training'||!Object.hasOwn(WEAPONS,m.weapon))return;const w=WEAPONS[m.weapon];p.inventory[m.weapon]={ammo:w.mag,reserve:999};if(w.slot==='primary')p.primary=m.weapon;else p.secondary=m.weapon;p.weapon=m.weapon;p.ammo=w.mag;p.reserve=999;p.reloadAt=0;send(p,{type:'purchased',weapon:p.weapon,money:p.money});return;}if(m.type==='marker'){if(!p.room||p.hp<=0||clock<(p.markerAt||0)||!['enemy','regroup'].includes(m.kind))return;p.markerAt=clock+2;const o={x:p.x,y:eye(p),z:p.z},d=direction(p.yaw,p.pitch),distance=Math.min(45,wallDistance(o,d,MAPS[p.room.map].boxes));const marker={type:'marker',id:p.id,kind:m.kind,name:p.name,position:{x:clamp(o.x+d.x*distance,-27,27),y:clamp(o.y+d.y*distance,.1,12),z:clamp(o.z+d.z*distance,-31,31)}};for(const ally of p.room.players.values())if(!ally.bot&&ally.team===p.team)send(ally,marker);return;}if(m.type==='rooms'){send(p,{type:'rooms',rooms:roomList()});return}if(m.type==='leave'){leave(p);send(p,{type:'left'});return}if(m.type==='grenade'){throwGrenade(p,p.room);return}if(m.type==='team'){changeTeam(p,m.team);return}if(m.type==='buy'){purchase(p,String(m.item));send(p,{type:'purchased',item:m.item,money:p.money,weapon:p.weapon});return}
...
function dealDamage(attacker,victim,r,damage,head,weapon,d){
 ...
 if(attacker&&attacker.team!==victim.team){attacker.stats??={shots:0,hits:0,headshots:0,damage:0};attacker.stats.damage+=Math.min(victim.hp,damage);}
...
function tick(){clock+=1/60;for(const r of rooms.values()){
 ...
 if(!isRoundMode(r.mode)&&r.restart&&clock>=r.restart){r.scores={blue:0,orange:0};r.flags=initialFlags();r.end=clock+r.minutes*60;r.restart=0;for(const p of r.players.values()){p.kills=0;p.deaths=0;p.captures=0;p.stats={shots:0,hits:0,headshots:0,damage:0};spawn(p,r)}broadcast(r,{type:'event',kind:'newmatch'})}
