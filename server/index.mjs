...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/api/health')return json(res,200,{ok:true,game:'Sector 16',version:'2.0.0',rooms:rooms.size,players:[...peers].filter(p=>p.room).length,tickRate:60});
...
function spawn(p,r,preserve=false){
 ...
 const retained=preserve&&p.hp>0;const owned=retained?Object.keys(p.inventory):isRoundMode(r.mode)?['pistol']:modeLoadout(r.mode,p.kills);
