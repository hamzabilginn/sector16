...
const rooms=new Map(),peers=new Set(),limits=new Map();let clock=0;
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/api/health')return json(res,200,{ok:true,game:'Sector 16',version:'1.5.0',rooms:rooms.size,players:[...peers].filter(p=>p.room).length,tickRate:60});
...
function spawn(p,r,preserve=false){
 ...
 const retained=preserve&&p.hp>0;const owned=retained?Object.keys(p.inventory):isRoundMode(r.mode)?['pistol']:['rifle','pistol'];
 ...
 Object.assign(p,freshBody(pos[0]+(Math.random()-.5),pos[1],p.team==='blue'?0:Math.PI),{ai:p.bot?makeBrain(p.botIndex||0):undefined,grenades:1,botTarget:null,botSeenAt:0,hp:100,armor:retained?p.armor:0,money:p.money??800,primary,secondary,weapon,inventory,ammo:inventory[weapon].ammo,reserve:inventory[weapon].reserve,reloadAt:0,nextFire:clock+.5,invuln:clock+2,respawn:0,queue:[],input:{},generation:(p.generation||0)+1});
}
...
function beginRound(r,fresh=false){if(fresh){r.round=0;r.scores={blue:0,orange:0};r.loss={blue:0,orange:0};for(const p of r.players.values()){p.money=800;p.hp=0;p.kills=0;p.deaths=0}}
...
function changeTeam(p,requested){
 const r=p.room;if(!r)throw Error('Önce bir odaya katıl.');
...
wss.on('connection',(ws,req)=>{const p={id:crypto.randomBytes(8).toString('hex'),ws,ip:clientIp(req),room:null,alive:true,ping:0,queue:[],input:{},ack:0,lastSeq:0,busy:false};peers.add(p);ws.on('pong',()=>p.alive=true);send(p,{type:'hello',id:p.id,rooms:roomList()});ws.on('message',async data=>{let m;try{m=JSON.parse(data);if(!m||typeof m.type!=='string')return;
 ...
 consume(p,'messages',60,10);if(m.type==='rooms'){send(p,{type:'rooms',rooms:roomList()});return}if(m.type==='leave'){leave(p);send(p,{type:'left'});return}if(m.type==='grenade'){throwGrenade(p,p.room);return}if(m.type==='team'){changeTeam(p,m.team);return}if(m.type==='buy'){purchase(p,String(m.item));send(p,{type:'purchased',item:m.item,money:p.money,weapon:p.weapon});return}
 if(p.busy)throw Error('İşlem sürüyor.');p.busy=true;
 try{if(m.type==='create'){consume(p,'create',6,60);if(p.room)throw Error('Zaten bir odadasın.');if(rooms.size>=40)throw Error('Sunucu oda sınırına ulaştı.');chooseTeam({players:new Map(),maxPlayers:2},m.team);const name=String(m.name||'').trim();if(name.length<2||name.length>36)throw Error('Oda adı 2–36 karakter olmalı.');const raw=String(m.password||'');if(raw.length>64)throw Error('Şifre en fazla 64 karakter olmalı.');let password=null;if(raw){const salt=crypto.randomBytes(16).toString('hex');password={salt,hash:Buffer.from(await scrypt(raw,salt,32)).toString('hex')}}if(ws.readyState!==WebSocket.OPEN)return;const r=makeRoom({name,maxPlayers:[2,8,12,16].includes(Number(m.maxPlayers))?Number(m.maxPlayers):8,minutes:[5,8,15].includes(Number(m.minutes))?Number(m.minutes):8,bots:m.bots!==false,botCount:m.botCount,botDifficulty:m.botDifficulty,password,map:m.map,mode:m.mode});enter(p,r,m.nickname,m.team)}
 else if(m.type==='join'){consume(p,'join',20,60);const r=rooms.get(String(m.room));if(!r)throw Error('Oda artık mevcut değil.');if(r.password){const raw=String(m.password||'');if(raw.length>64)throw Error('Şifre hatalı.');const hash=await scrypt(raw,r.password.salt,32);if(!crypto.timingSafeEqual(Buffer.from(r.password.hash,'hex'),hash))throw Error('Şifre hatalı.')}if(ws.readyState===WebSocket.OPEN)enter(p,r,m.nickname,m.team)}
 else if(m.type==='quick'){let r=[...rooms.values()].find(r=>!r.password&&[...r.players.values()].filter(p=>!p.bot).length<r.maxPlayers);if(!r){if(rooms.size>=40)throw Error('Sunucu dolu.');r=makeRoom()}enter(p,r,m.nickname,m.team)}}finally{p.busy=false}
 }catch(e){send(p,{type:'error',message:e.message||'İşlem tamamlanamadı.'})}});ws.on('error',()=>{});ws.on('close',()=>{leave(p);peers.delete(p)});});
function botInput(p,r){return tacticalInput(p,r,clock)}
function throwGrenade(p,r){if(!r||p.hp<=0||p.reloadAt||!p.grenades||r.restart||(isRoundMode(r.mode)&&r.phase!=='live'))return;p.grenades--;if(p.input)p.input.interact=false;p.invuln=Math.min(p.invuln,clock);r.grenades.push(createGrenade(p,clock,crypto.randomBytes(5).toString('hex')));p.nextFire=Math.max(p.nextFire,clock+.55);broadcast(r,{type:'grenadethrown',id:p.id});}
function tickGrenades(r){for(let i=r.grenades.length-1;i>=0;i--){const g=r.grenades[i];stepGrenade(g,1/60,MAPS[r.map].boxes);if(clock<g.explodeAt)continue;r.grenades.splice(i,1);broadcast(r,{type:'explosion',position:{x:g.x,y:g.y,z:g.z},kind:'grenade'});const owner=r.players.get(g.owner)||{id:g.owner,name:g.name,team:g.team,kills:0,money:0};for(const v of r.players.values()){if(v.hp<=0||v.invuln>clock||(v.team===g.team&&v.id!==g.owner))continue;const damage=blastDamage(g,v,MAPS[r.map].boxes);if(damage)dealDamage(owner,v,r,damage,false,'grenade',{x:v.x-g.x,y:.3,z:v.z-g.z});}}}
...
function tick(){clock+=1/60;for(const r of rooms.values()){
 ...
 if(!isRoundMode(r.mode)&&r.restart&&clock>=r.restart){r.scores={blue:0,orange:0};r.flags=initialFlags();r.end=clock+r.minutes*60;r.restart=0;for(const p of r.players.values()){p.kills=0;p.deaths=0;p.captures=0;spawn(p,r)}broadcast(r,{type:'event',kind:'newmatch'})}
 if(!isRoundMode(r.mode)&&!r.restart&&(clock>=r.end||r.scores.blue>=(r.mode==='ctf'?5:50)||r.scores.orange>=(r.mode==='ctf'?5:50))){r.restart=clock+2;broadcast(r,{type:'end',winner:r.scores.blue===r.scores.orange?'draw':r.scores.blue>r.scores.orange?'blue':'orange'})}
 ...
 if(p.bot&&i.grenade)throwGrenade(p,r);simulate(p,i,1/60,MAPS[r.map].boxes);if(i.weapon&&p.inventory[i.weapon]&&i.weapon!==p.weapon){p.inventory[p.weapon]={ammo:p.ammo,reserve:p.reserve};p.weapon=i.weapon;Object.assign(p,p.inventory[p.weapon]);p.reloadAt=0;p.nextFire=Math.max(p.nextFire,clock+.3)}
...
const snapshots=setInterval(()=>{for(const r of rooms.values()){if(!r.players.size)continue;const players=[...r.players.values()].map(p=>({id:p.id,name:p.name,team:p.team,bot:!!p.bot,role:p.bot?BOT_ROLES[(p.botIndex||0)%BOT_ROLES.length]:null,grenades:p.grenades,x:p.x,y:p.y,z:p.z,yaw:p.yaw,pitch:p.pitch,vx:p.vx,vy:p.vy,vz:p.vz,ground:p.ground,jumpHeld:p.jumpHeld,crouch:p.crouch,hp:p.hp,inBase:inOwnBase(p),canBuy:p.hp>0&&inOwnBase(p)&&!r.restart&&(!isRoundMode(r.mode)||r.phase!=='matchEnd'),armor:p.armor,money:p.money,captures:p.captures||0,primary:p.primary,secondary:p.secondary,weapon:p.weapon,ammo:p.ammo,reserve:p.reserve,reload:Math.max(0,p.reloadAt-clock),reloadEnd:p.reloadAt,kills:p.kills,deaths:p.deaths,ping:Math.round(p.ping),ack:p.ack,invuln:p.invuln>clock,respawn:Math.max(0,p.respawn-clock),generation:p.generation}));broadcast(r,{type:'state',time:clock,map:r.map,mode:r.mode,flags:r.mode==='ctf'?r.flags:undefined,bomb:r.mode==='bomb'?r.bomb:undefined,grenades:r.grenades.map(g=>({id:g.id,x:g.x,y:g.y,z:g.z})),phase:r.phase,round:r.round,roundTarget:ROUND_TARGET,remaining:Math.max(0,(r.mode==='bomb'&&r.bomb?.state==='planted'?r.bomb.detonateAt:isRoundMode(r.mode)?r.phaseEnd:r.end)-clock),restart:Math.max(0,r.restart-clock),scores:r.scores,players})}},1000/20);
