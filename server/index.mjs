...
function spawn(p,r,preserve=false){
 ...
 Object.assign(p,freshBody(pos[0]+(Math.random()-.5),pos[1],p.team==='blue'?0:Math.PI),{ai:p.bot?makeBrain(p.botIndex||0):undefined,medkits:1,grenades:3,utility:{he:1,flash:1,smoke:1},botTarget:null,botSeenAt:0,hp:100,armor:retained?p.armor:0,money:p.money??800,primary,secondary,weapon,inventory,ammo:inventory[weapon].ammo,reserve:inventory[weapon].reserve,reloadAt:0,nextFire:clock+.5,invuln:clock+2,respawn:0,queue:[],input:{},generation:(p.generation||0)+1});
}
...
function tick(){clock+=1/60;for(const r of rooms.values()){
 ...
 if(!isRoundMode(r.mode)&&r.restart&&clock>=r.restart){r.matchId++;r.grenades=[];r.smokes=[];r.history=[];r.bomb=null;r.scores={blue:0,orange:0};r.flags=initialFlags();r.end=clock+r.minutes*60;r.restart=0;for(const p of r.players.values()){p.kills=0;p.deaths=0;p.captures=0;p.stats={shots:0,hits:0,headshots:0,damage:0};spawn(p,r)}broadcast(r,{type:'event',kind:'newmatch'})}
