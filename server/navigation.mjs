...
export function navigation(map){if(cache.has(map))return cache.get(map);const boxes=MAPS[map].boxes,nodes=[];
 for(let z=-30;z<=30;z+=STEP)for(let x=-26;x<=26;x+=STEP){if(!boxes.some(b=>b.y-b.h/2<1.7&&Math.abs(x-b.x)<b.w/2+.50&&Math.abs(z-b.z)<b.d/2+.50))nodes.push({x,z});}
