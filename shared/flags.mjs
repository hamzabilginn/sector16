...
export function initialFlags(){return Object.fromEntries(Object.entries(FLAG_BASES).map(([team,pos])=>[team,{team,...pos,carrier:null,returnAt:0}]))}
export function resetFlag(flag){Object.assign(flag,FLAG_BASES[flag.team],{carrier:null,returnAt:0})}
...
export function updateFlags(room,time,emit){if(room.restart)return;const flags=room.flags;if(!flags)return;
 ...
 if(enemy.carrier===p.id&&!own.carrier&&!own.returnAt&&near(FLAG_BASES[p.team])){room.scores[p.team]++;p.captures=(p.captures||0)+1;resetFlag(enemy);emit({type:'flag',kind:'capture',team:p.team,name:p.name})}
 }
}
