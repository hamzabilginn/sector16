...
export function bombSites(map){return MAPS[map]?.bombSites||BOMB_SITES}
...
export function updateBomb(room,time,emit=()=>{}){
 ...
 if(b.state==='carried'){
  const p=room.players.get(b.carrier),site=p&&bombSites(room.map).find(s=>Math.hypot(p.x-s.x,p.z-s.z)<=s.radius&&p.y<.4);
